...
/**
 * The Audit4j Context. This will load and execute required resources in to the memory when initializing audit4j,
 * Context makes sure necessary resources provide when running audit4j and it's plugins. Also this will ensure to
 ...
 * <li>{@link #initWithConfiguration(Configuration configuration)} Initialize the Audit4j context with external
 * configurations.</li>
 * <li>{@link #initWithConfiguration(String configFilePath)} Initialize the Audit4j context with external configuration
 * file.</li>
 ...
 * <li>{@link #setConfig(Configuration configuration)} set initial and external configurations in to context.</li>
 * </ul>
 *
...
public final class Context {
	...
	/**
	 * Initialize the Audit4j instance. This will ensure the single audit4j instance and single Configuration repository
	 * load in to the memory.
	 */
	final static void init() {
		...
		if (lifeCycle.getStatus().equals(RunStatus.READY) || lifeCycle.getStatus().equals(RunStatus.STOPPED)) {
			final Audit4jBanner banner = new Audit4jBanner();
			banner.printBanner();
			...
			if (conf.getProperties() == null) {
				...
				for (final Map.Entry<String, String> entry : conf.getProperties().entrySet()) {
			...
			final DefaultAnnotationTransformer defaultAnnotationTransformer = new DefaultAnnotationTransformer();
			final ObjectSerializerCommand serializerCommand = (ObjectSerializerCommand) PreConfigurationContext.getCommandByName("-objectSerializer");
			...
			for (final AuditEventFilter filter : conf.getFilters()) {
			...
			if (conf.getJmx() != null) {
				final MBeanAgent agent = new MBeanAgent();
			...
			stopWatch.stop();
			final Long initializationTime = stopWatch.getLastTaskTimeMillis();
	 ...
	 * @since 2.3.1
	 */
	public static void initWithConfiguration(final Configuration configuration) {
	 ...
	 * @since 2.3.1
	 */
	public static void initWithConfiguration(final String configFilePath) {
	...
	/**
	 * The Audit4j context shutdown method. This will release the all allocated resources by the Audit4j Context
	 * initialization.
	 *
	 * @since 2.2.0
	 */
	...
	private final static void checkEnvironment() {
		...
		// Check java support.!
		final boolean javaSupport = EnvUtil.isJDK7OrHigher();
		if (!javaSupport) {
	...
	private static void loadRegistry() {
		...
		for (final AuditEventFilter filter : PreConfigurationContext.getPrefilters()) {
			configContext.addFilter(filter);
		}
		// Load audit annotation filters to runtime configurations.
		for (final AuditAnnotationFilter annotationFilter : PreConfigurationContext.getPreannotationfilters()) {
	...
	private static void initHandlers() {
		...
		for (final Handler handler : conf.getHandlers()) {
			try {
				if (!configContext.getHandlers().contains(handler)) {
					final Map<String, String> handlerproperties = new HashMap<>();
	...
	private static void initStreams() {
		...
		final MetadataCommand command = (MetadataCommand) PreConfigurationContext.getCommandByName("-metadata");
		if (command.isAsync()) {
			final AsyncAnnotationAuditOutputStream asyncAnnotationStream = new AsyncAnnotationAuditOutputStream(new AuditProcessOutputStream(Context.getConfigContext()), annotationTransformer);
			final MetadataLookupStream metadataStream = new MetadataLookupStream(new AuditProcessOutputStream(Context.getConfigContext()));
			final AsyncAuditOutputStream asyncStream = new AsyncAuditOutputStream(metadataStream, asyncAnnotationStream);
			auditStream = new AuditEventOutputStream(asyncStream);
		}
		else {
			final AsyncAnnotationAuditOutputStream asyncAnnotationStream = new AsyncAnnotationAuditOutputStream(new AuditProcessOutputStream(Context.getConfigContext()), annotationTransformer);
			final AsyncAuditOutputStream asyncStream = new AsyncAuditOutputStream(new AuditProcessOutputStream(Context.getConfigContext()), asyncAnnotationStream);
			final MetadataLookupStream metadataStream = new MetadataLookupStream(asyncStream);
	 ...
	 *            the new config
	 */
	final static void setConfig(final Configuration conf) {
